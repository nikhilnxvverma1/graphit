...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "com.nikhilnxvverma1.graphit_mac" in the user's Application Support directory.
    NSURL *appSupportURL = [[[NSFileManager defaultManager] URLsForDirectory:NSApplicationSupportDirectory inDomains:NSUserDomainMask] lastObject];
    return [appSupportURL URLByAppendingPathComponent:@"com.nikhilnxvverma1.graphit_mac"];
}
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notified) name:NSPersistentStoreCoordinatorStoresDidChangeNotification object:nil];
    
    if (!shouldFail && !error) {
        NSPersistentStoreCoordinator *coordinator = [[NSPersistentStoreCoordinator alloc] initWithManagedObjectModel:[self managedObjectModel]];
        NSURL *url = [applicationDocumentsDirectory URLByAppendingPathComponent:@"OSXCoreDataObjC.storedata"];
        NSDictionary *storeOptions =@{NSPersistentStoreUbiquitousContentNameKey: @"GraphItStore"};
